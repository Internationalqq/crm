...
Дата: 19 сентября 2026 года. Основание — запрос пользователя на большой модуль реальных цен по всем позициям. **Статус: проект реализации и проверенный разбор ограничений. Пакеты A–F ниже ещё не реализованы.** Исходные опубликованные версии: Автобот — `99afae4`, CRM — `59cb3de`. Обычный поиск остаётся без Авито и Hermes.
...
| Обнаружение источников | Находит реальные URL товара, услуги, прайса; не подтверждает цены | Адаптер официального поиска + существующие каталоги и HTTP-поиск |
...
Основной кандидат для широкого поиска по России — **официальный Yandex Search API**. Он возвращает адреса и описания результатов. Цену затем проверяем на сайте поставщика. Регион в запросе и параметрах ранжирования помогает поиску, но не доказывает доставку или работу продавца в этом регионе.

Каналы работают последовательно по оставшимся пробелам: свежий совместимый индекс → подключённые каталоги → поисковый API → другие варианты запроса/прайсы. Наличие непригодных ссылок из каталога не останавливает переход дальше. Дорогие или спорные позиции получают дополнительные независимые источники в пределах общего лимита. Несколько страниц одного продавца не считаются разными поставщиками.
...
По официальной странице тарифов, проверенной 19.09.2026, дневной синхронный поиск Яндекса стоит **488 ₽ за 1 000 запросов**, отложенный — **30,5 ₽ за 1 000**; суммы на странице указаны с НДС. Отложенный режим возвращает идентификатор операции, результат запрашивается после её завершения. Его реальную задержку и пригодность для наших заданий нужно измерить при подключении.

Пример для 100 различных подготовленных позиций по три поисковых запроса: 300 запросов, **146,40 ₽** синхронно или **9,15 ₽** отложенно по дневному тарифу. Это иллюстрация цены обнаружения ссылок, не обещание найти 100 цен и не полная стоимость эксплуатации. Не включены повторные запросы, дополнительные страницы выдачи, обработка документов, сервер и необязательный ИИ. Тарифы перед включением перепроверяются.

Рекомендация: для всей сметы испытать отложенный режим; для единичного срочного поиска — синхронный. Хранить ID операции, чтобы восстановление сервера продолжало ожидание, а не отправляло платный запрос повторно. На первом испытании сравнить оба режима на одинаковых позициях; более дешёвый тариф сам по себе не доказывает нужную скорость и релевантность.

Перед платным запуском нужны выбранный лимит и доступ к сервису. Лимиты на запуск/день/месяц хранятся явно; без заданного бюджета платный канал выключен. Резерв расходов производится атомарно перед запросом. Таймаут после отправки учитывается как потенциально оплаченный; повтор ограничен. По достижении лимита сохраняется частичный результат с причиной остановки. Ключ хранится на сервере и не попадает в API-ответы, DOM или логи.

Бюджет уточнён отдельным вопросом; на момент подготовки проекта ответ не получен. Подписки, аккаунты и платные запросы не создавались. Рабочий каталог остаётся доступным независимо от будущего подключения API.

Источники: [тарифы Яндекса](https://aistudio.yandex.ru/ru/docs/search-api/pricing), [REST WebSearch.Search](https://aistudio.yandex.ru/ru/docs/search-api/api-ref/WebSearch/search), [синхронный и отложенный поиск](https://aistudio.yandex.ru/ru/docs/search-api/concepts/web-search). Вариант резервного провайдера — [Brave Search API](https://brave.com/search/api/), текущая цена поиска $5/1 000 запросов; качество на русских строительных позициях в этой работе не измерялось.
...
| **C. Широкое обнаружение** | Поисковый API и каталоги дополняют друг друга; после неудачных кандидатов поиск идёт дальше | Живой поиск вне трёх каталогов; повтор операции не создаёт лишний платный запрос; лимит и недоступность провайдера видны; нужен настроенный доступ |
...
Пакеты A и B не зависят от платного доступа и идут первыми. C–D расширяют охват на проверяемых позициях, E использует готовые контракты, F проверяет весь пользовательский сценарий. Завершение одного пакета не означает готовность всего модуля.
...
Прикладной код, интерфейс и рабочие данные этим пакетом не меняются. Полные Python/frontend-наборы, browser viewports и живое качество поискового API не проверялись: реализация ещё впереди. Самопроверка документа не является независимым ревью.

**Следующий конкретный шаг — пакет A:** паспорт позиции и исправление классификации/потери характеристик на контрольных случаях, затем подключение паспорта к существующему поиску и индексу с проверкой сохранения старых контрактов. Отдельный вопрос к пользователю — потолок расходов поискового API; без ответа платные операции остаются выключенными.
