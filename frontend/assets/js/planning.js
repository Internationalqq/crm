(function () {
    ...
    function canEditProductionSchedule() {
        var user = state && (state.currentUser || state.user);
        return !!user && !user.isGuest && !user.isPublic && !hasRole('guest');
    }
    ...
    function scheduleProcurementTiming(alert) {
        ...
        var owner = supervisor && alert.responsibleUserName ? (alert.responsibleUserName + ': ') : '';
        if (supervisor && alert.needsAssignment) owner = 'ответственный не назначен • ';
        if (Number(alert.daysUntilOrder) < 0) return owner + (personal ? 'вам нужно было заказать ' : 'срок заказа вышел ') + Math.abs(Number(alert.daysUntilOrder)) + ' дн. назад';
        if (Number(alert.daysUntilOrder) === 0) return owner + (personal ? 'вам заказать сегодня' : 'заказ сегодня');
        return owner + (personal ? 'вам заказать в течение ' : 'заказ в течение ') + Number(alert.daysUntilOrder) + ' дн.';
    }
    ...
    function materialModalQuantityMeta(item) {
        var plan = quantityPlanInfo(item);
        var unit = plan.unit || (item && item.unit) || 'ед.';
    ...
    function renderProductionOperationEditor(project, schedule) {
        ...
        return '<div class="production-operation-overlay" data-production-editor-overlay aria-hidden="true"></div>' +
            '<aside class="production-operation-drawer" data-production-editor aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="production-operation-editor-title">' +
                ...
                '<header class="production-operation-editor-head"><span class="eyebrow">Производственный график</span><h3 id="production-operation-editor-title" data-production-editor-title>Новая работа</h3><p data-production-editor-description>Операция может быть связана с одной или несколькими строками сметы — работами или материалами.</p></header>' +
                '<form class="production-operation-form" data-production-operation-form data-project-id="' + escapeHtml(project.id) + '" novalidate>' +
                    '<input type="hidden" name="operation_id" value="">' +
                    '<label class="production-form-wide" data-production-title-field><span data-production-title-label>Наименование работы</span><input name="title" required maxlength="500" autocomplete="off"></label>' +
                    '<div class="production-form-columns production-form-columns-volume" data-production-volume-fields>' +
                    ...
                    '</div>' +
                    '<div class="production-form-columns production-form-columns-team" data-production-team-fields>' +
                    ...
                    '</div>' +
                    '<div class="production-form-columns production-form-columns-schedule">' +
                        '<label><span>Начало работы</span><input name="start_date" type="date" min="' + escapeHtml(productionScheduleStartDate(project, schedule)) + '" max="2200-12-31"></label>' +
                        '<label><span>Продолжительность, дней</span><input name="duration_days" type="number" min="0.5" max="3650" step="0.5" value="0.5"></label>' +
                    '</div>' +
                    '<div class="production-start-placement-help"><span>Укажите дату, чтобы запустить эту работу параллельно с другими.</span><button class="ghost compact" type="button" data-production-operation-start-reset hidden>Вернуть по порядку</button></div>' +
                    '<div class="production-form-columns production-form-columns-state" data-production-state-fields>' +
                        '<label><span>Состояние</span><select name="status"><option value="draft">Черновик</option><option value="needs_review">Нужна проверка</option><option value="confirmed">Подтверждено</option><option value="in_progress">В работе</option><option value="completed">Завершено</option><option value="on_hold">На паузе</option><option value="linked">Связано со сметой</option><option value="outside_estimate">Вне сметы</option><option value="stale">Связи устарели</option></select></label>' +
                        '<label><span>Цвет на графике</span><select name="color"><option value="slate">Серый</option><option value="blue">Синий</option><option value="teal">Бирюзовый</option><option value="green">Зелёный</option><option value="violet">Фиолетовый</option><option value="rose">Красный</option></select></label>' +
                    '</div>' +
                    '<fieldset class="production-estimate-links" data-production-estimate-links><legend>Связи со сметой</legend><p>Выберите всё, на основании чего появилась операция. Эти связи используются только в графике и не меняют смету.</p>' +
                    ...
                    '<footer class="production-operation-form-actions"><button class="ghost" type="button" data-production-editor-close>Отмена</button><button class="primary" type="submit" data-production-editor-submit><span>Сохранить работу</span></button></footer>' +
                '</form>' +
            '</aside>' +
            ...
            '<section class="production-group-editor" data-production-group-editor aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="production-group-editor-title">' +
                ...
                '<form class="production-operation-form production-group-editor-form" data-production-group-editor-form novalidate>' +
                    ...
                    '<label class="production-estimate-label-toggle" data-production-estimate-label-field hidden><input type="checkbox" name="show_estimate_label" checked><span><b>Показывать подпись «Смета»</b><small>Название сметы останется без изменений.</small></span></label>' +
                    '<div class="production-section-volume-fields" data-production-section-volume-fields hidden>' +
                        '<div class="production-form-columns production-form-columns-volume">' +
                            '<label><span>Сводный объём раздела</span><input name="planned_qty" type="text" inputmode="decimal" autocomplete="off" placeholder="Например, 120"></label>' +
                            '<label><span>Ед. изм.</span><input name="unit" maxlength="40" autocomplete="off" placeholder="м²"></label>' +
                        '</div>' +
                        '<label><span>Продолжительность раздела, дней</span><input name="section_duration_days" type="number" min="0.5" max="3650" step="0.5" value="0.5"></label>' +
                        '<label><span>Начало раздела</span><input name="section_start_date" type="date" min="1900-01-01" max="2200-12-31"></label>' +
                        '<div class="production-start-placement-help"><span>Раздел можно начать раньше и вести параллельно с соседними.</span><button class="ghost compact" type="button" data-production-section-start-reset hidden>Вернуть по порядку</button></div>' +
                        '<p class="production-section-volume-help">Новая длительность распределится между работами раздела и сдвинет следующие работы графика.</p>' +
                        '<p class="production-section-volume-help" data-production-section-volume-help>Это значение показывается в строке раздела и не меняет объёмы отдельных работ.</p>' +
                        '<button class="ghost compact production-section-volume-reset" type="button" data-production-section-volume-reset hidden>Вернуть расчёт по работам</button>' +
                    '</div>' +
                    '<div class="production-operation-form-error" data-production-group-editor-error role="alert" aria-live="polite"></div>' +
                    '<footer class="production-operation-form-actions"><button class="danger production-delete-section-button" type="button" data-production-delete-section hidden><i data-lucide="trash-2" aria-hidden="true"></i><span>Удалить раздел</span></button><button class="ghost" type="button" data-production-group-editor-close>Отмена</button><button class="primary" type="submit">Сохранить</button></footer>' +
                '</form>' +
            '</section>';
    }
    ...
    function productionScheduleIsoDate(value) {
    ...
    function productionScheduleTodayIso(schedule) {
        var supplied = productionScheduleIsoDate(schedule && schedule.today);
    ...
    function productionScheduleStartDate(project, schedule) {
        return productionScheduleIsoDate(schedule && (schedule.startDate || schedule.start_date)) ||
            productionScheduleIsoDate(project && (project.started_at || project.startDate || project.start_date)) ||
            productionScheduleTodayIso(schedule);
    }

    function productionScheduleAddDays(isoDate, offset) {
        var normalized = productionScheduleIsoDate(isoDate);
        if (!normalized) return '';
        var parts = normalized.split('-').map(Number);
        var value = new Date(Date.UTC(parts[0], parts[1] - 1, parts[2] + Number(offset || 0)));
    ...
    function productionScheduleDayMeta(startDate, dayNumber, todayIso) {
        ...
        var monthShort = ['янв.', 'февр.', 'мар.', 'апр.', 'мая', 'июн.', 'июл.', 'авг.', 'сент.', 'окт.', 'нояб.', 'дек.'];
        var weekdays = ['вс', 'пн', 'вт', 'ср', 'чт', 'пт', 'сб'];
        var weekdayIndex = value.getUTCDay();
        return {
            iso: isoDate,
            dayNumber: Number(dayNumber),
            calendarDay: value.getUTCDate(),
            weekday: weekdays[weekdayIndex],
            monthKey: String(value.getUTCFullYear()) + '-' + String(value.getUTCMonth() + 1).padStart(2, '0'),
            monthLabel: monthNames[value.getUTCMonth()] + ' ' + String(value.getUTCFullYear()),
            shortLabel: String(value.getUTCDate()) + ' ' + monthShort[value.getUTCMonth()],
            isWeekend: weekdayIndex === 0 || weekdayIndex === 6,
            isToday: isoDate === productionScheduleIsoDate(todayIso)
        };
    }

    function productionScheduleMonthGroups(startDate, firstDay, lastDay, todayIso) {
        var groups = [];
        for (var day = firstDay; day <= lastDay; day += 1) {
            var meta = productionScheduleDayMeta(startDate, day, todayIso);
            var current = groups.length ? groups[groups.length - 1] : null;
            if (!current || current.key !== meta.monthKey) {
                current = { key: meta.monthKey, label: meta.monthLabel, firstDay: day, dayCount: 0 };
                groups.push(current);
            }
            current.dayCount += 1;
        }
    ...
    function productionScheduleHealth(item) {
        var raw = String(item && (item.healthStatus || item.executionHealth || item.health_status) || '').trim().toLowerCase();
        var aliases = { success: 'green', warning: 'yellow', danger: 'red', critical: 'red', pending: 'neutral' };
        raw = aliases[raw] || raw;
        if (['neutral', 'green', 'yellow', 'red'].indexOf(raw) < 0) {
            var status = String(item && (item.status || item.linkStatus) || '').trim().toLowerCase();
            if (status === 'completed' || item && item.isCompleted) raw = 'green';
            else if (['review', 'needs_review', 'requires_review', 'unverified', 'stale', 'ambiguous', 'orphaned', 'outside', 'outside_estimate', 'unlinked'].indexOf(status) >= 0) raw = 'yellow';
            else raw = 'neutral';
        }
        var defaultLabels = {
            neutral: 'Ещё не началось',
            green: 'Идёт по графику',
            yellow: 'Нужна проверка',
            red: 'Есть отставание'
        };
        return {
            key: raw,
            label: String(item && (item.healthLabel || item.health_label) || defaultLabels[raw])
        };
    }
    ...
    function productionScheduleHiddenColumnSet(schedule) {
        var allowed = { people: true, shifts: true, brigades: true };
        var hidden = {};
        (Array.isArray(schedule && schedule.hiddenColumns) ? schedule.hiddenColumns : []).forEach(function (column) {
            var key = String(column || '').trim();
            if (allowed[key]) hidden[key] = true;
        });
        return hidden;
    }
    ...
    function productionScheduleSectionVolume(items) {
        ...
        (Array.isArray(items) ? items : []).forEach(function (item) {
            ...
            var unitKey = unit.toLowerCase().replace(/\s+/g, ' ');
            if (!Object.prototype.hasOwnProperty.call(totals, unitKey)) {
                totals[unitKey] = 0;
                labels[unitKey] = unit;
                order.push(unitKey);
            }
        ...
        if (order.length === 1) {
            return {
                plannedQty: totals[order[0]],
                unit: labels[order[0]],
                label: quantityText(totals[order[0]]) + ' ' + labels[order[0]]
            };
        }
        return {
            plannedQty: null,
            unit: '',
            label: order.length > 1
                ? String((items || []).length) + ' работ · разные единицы измерения'
                : String((items || []).length) + ' работ'
        };
    }

    function productionScheduleSectionOverride(schedule, estimate, section) {
        var sourceId = estimate && estimate.sourceId;
        var sectionTitle = String(section && section.rawTitle || '').trim().toLowerCase();
        return (Array.isArray(schedule && schedule.sectionOverrides) ? schedule.sectionOverrides : []).find(function (override) {
            return String(override && override.estimateSourceId) === String(sourceId) &&
                String(override && override.sectionTitle || '').trim().toLowerCase() === sectionTitle;
        }) || null;
    }

    function productionScheduleSectionSummary(estimate, section, index, schedule) {
        var filled = {};
        var automatic = {};
        var overridden = {};
        ...
        var calculatedVolume = productionScheduleSectionVolume(section.items);
        var volumeOverride = productionScheduleSectionOverride(schedule, estimate, section);
        var overrideQty = Number(volumeOverride && volumeOverride.plannedQty);
        var isVolumeOverridden = !!volumeOverride && Number.isFinite(overrideQty);
        var plannedQty = isVolumeOverridden ? overrideQty : calculatedVolume.plannedQty;
        var unit = isVolumeOverridden ? String(volumeOverride.unit || '').trim() : calculatedVolume.unit;
        var scheduleStartDate = productionScheduleStartDate(null, schedule);
        var plannedStartDate = filledSlots.length
            ? productionScheduleAddDays(scheduleStartDate, Math.floor((filledSlots[0] - 1) / 2))
            : '';
        var autoStartDate = automaticSlots.length
            ? productionScheduleAddDays(scheduleStartDate, Math.floor((automaticSlots[0] - 1) / 2))
            : plannedStartDate;
        var startIsOverridden = (section.items || []).some(function (item) {
            return String(item && (item.placementMode || item.placement_mode) || '').toLowerCase() === 'manual';
        });
        var manualSectionItems = (section.items || []).filter(function (item) { return !!(item && item.isManualSection); });
        return {
            ...
            unit: unit,
            volumeLabel: isVolumeOverridden
                ? quantityText(overrideQty) + (unit ? ' ' + unit : '')
                : calculatedVolume.label,
            calculatedPlannedQty: calculatedVolume.plannedQty,
            calculatedUnit: calculatedVolume.unit,
            isVolumeOverridden: isVolumeOverridden,
            ...
            isStartOverridden: startIsOverridden,
            placementMode: startIsOverridden ? 'manual' : 'auto',
            manualOperationId: manualSectionItems.length === 1 && section.items.length === 1 ? productionOperationId(manualSectionItems[0]) : null,
    ...
    function productionSchedulePrintItems(schedule, viewMode) {
        ...
        productionScheduleGroups(items).forEach(function (estimate) {
            estimate.sections.forEach(function (section) {
                var summary = productionScheduleSectionSummary(estimate, section, summaryIndex += 1, schedule);
    ...
    function productionScheduleLegendMarkup(className) {
        ...
        return '<div class="' + prefix + '" aria-label="Статусы выполнения графика">' +
            '<span><i class="is-neutral" aria-hidden="true"></i>Ещё не началось</span>' +
            '<span><i class="is-green" aria-hidden="true"></i>По графику</span>' +
            '<span><i class="is-yellow" aria-hidden="true"></i>Нужна проверка</span>' +
            '<span><i class="is-red" aria-hidden="true"></i>Отставание или просрочка</span>' +
        '</div>';
    }
    ...
    function productionSchedulePrintConfig(schedule, options) {
        ...
        var fixedWidth = 154 - (hiddenColumns.people ? 14 : 0) - (hiddenColumns.shifts ? 14 : 0) - (hiddenColumns.brigades ? 16 : 0);
        var daysPerSheet = dayCount;
        if (layout === 'paged') {
            var scale = scalePercent / 100;
            var availableNaturalWidth = 281 / scale;
            daysPerSheet = Math.floor((availableNaturalWidth - fixedWidth) / 5);
    ...
    function productionSchedulePrintDocument(project, schedule, options) {
        ...
        for (var sheetIndex = 0; sheetIndex < sheetCount; sheetIndex += 1) {
            ...
            var monthHeaders = productionScheduleMonthGroups(startDate, firstDay, lastDay, todayIso).map(function (group) {
                return '<th class="production-print-month" colspan="' + String(group.dayCount) + '">' + escapeHtml(group.label) + '</th>';
            }).join('');
            var dayHeaders = '';
            var dayColumns = '';
            for (var day = firstDay; day <= lastDay; day += 1) {
                var dayMeta = productionScheduleDayMeta(startDate, day, todayIso);
                dayHeaders += '<th class="production-print-day' + (dayMeta.isWeekend ? ' is-weekend' : '') + (dayMeta.isToday ? ' is-today' : '') + '" title="' + escapeHtml(dayMeta.shortLabel + ' · ' + dayMeta.weekday) + '">' + String(dayMeta.calendarDay) + '</th>';
                dayColumns += '<col class="production-print-slot-column">';
            }
            ...
            items.forEach(function (item, itemIndex) {
                ...
                if (sectionTitle && sectionTitle !== previousSection) {
                    rows.push('<tr class="production-print-section"><th colspan="' + String(fixedColumnCount + (lastDay - firstDay + 1)) + '">' + escapeHtml(sectionTitle) + '</th></tr>');
                ...
                for (var cellDay = firstDay; cellDay <= lastDay; cellDay += 1) {
                    ...
                    var filledSlotCount = (filled[String(firstSlotNumber)] ? 1 : 0) + (filled[String(secondSlotNumber)] ? 1 : 0);
                    var isOverridden = !!overridden[String(firstSlotNumber)] || !!overridden[String(secondSlotNumber)];
                    cells += '<td class="production-print-slot' + (filledSlotCount ? ' is-filled tone-' + health.key : '') + (filledSlotCount === 1 ? ' is-partial' : '') + (isOverridden ? ' is-overridden' : '') + '" aria-label="' + escapeHtml(health.label) + '"></td>';
                }
                ...
                rows.push('<tr class="production-print-work">' +
                    ...
                    '<th class="production-print-title">' + escapeHtml(item.title || 'Работа') + '<small class="production-print-health is-' + health.key + '">' + escapeHtml(health.label) + '</small></th>' +
                    '<td>' + escapeHtml(volume) + '</td>' +
                    (hiddenColumns.people ? '' : '<td>' + escapeHtml(String(people)) + '</td>') +
                    (hiddenColumns.shifts ? '' : '<td>' + escapeHtml(String(shifts)) + '</td>') +
                    (hiddenColumns.brigades ? '' : '<td>' + escapeHtml(String(brigades)) + '</td>') +
                    '<td>' + escapeHtml(quantityText(duration)) + '</td>' + cells + '</tr>');
            });
            if (!rows.length) {
                rows.push('<tr class="production-print-empty"><td colspan="' + String(fixedColumnCount + (lastDay - firstDay + 1)) + '">График пока пуст</td></tr>');
            }

            var firstDateMeta = productionScheduleDayMeta(startDate, firstDay, todayIso);
            var lastDateMeta = productionScheduleDayMeta(startDate, lastDay, todayIso);
            var naturalWidth = fixedWidth + (lastDay - firstDay + 1) * 5;
            sheets.push('<section class="production-print-sheet"><div class="production-print-canvas" data-production-print-canvas style="--production-print-natural-width:' + String(naturalWidth) + 'mm">' +
                '<header class="production-print-sheet-head"><div><span>График производства работ</span><h1>' + escapeHtml(projectTitle) + '</h1>' + (projectAddress ? '<p>' + escapeHtml(projectAddress) + '</p>' : '') + '</div>' +
                    '<div class="production-print-range"><b>Дни ' + String(firstDay) + (firstDay === lastDay ? '' : '–' + String(lastDay)) + '</b><span>' + escapeHtml(firstDateMeta.shortLabel + (firstDay === lastDay ? '' : ' — ' + lastDateMeta.shortLabel)) + ' · часть ' + String(sheetIndex + 1) + ' из ' + String(sheetCount) + '</span></div></header>' +
                productionScheduleLegendMarkup('production-print-legend') +
                '<table class="production-print-table"><colgroup><col class="production-print-number-column"><col class="production-print-title-column"><col class="production-print-volume-column">' + (hiddenColumns.people ? '' : '<col class="production-print-people-column">') + (hiddenColumns.shifts ? '' : '<col class="production-print-shifts-column">') + (hiddenColumns.brigades ? '' : '<col class="production-print-brigades-column">') + '<col class="production-print-duration-column">' + dayColumns + '</colgroup>' +
                    '<thead><tr><th rowspan="2">№</th><th rowspan="2">Наименование работ</th><th rowspan="2">Объём</th>' + (hiddenColumns.people ? '' : '<th rowspan="2">Чел.</th>') + (hiddenColumns.shifts ? '' : '<th rowspan="2">Смен</th>') + (hiddenColumns.brigades ? '' : '<th rowspan="2">Бригад</th>') + '<th rowspan="2">Дней</th>' + monthHeaders + '</tr><tr>' + dayHeaders + '</tr></thead>' +
                    '<tbody>' + rows.join('') + '</tbody></table>' +
                '<footer class="production-print-footer"><span>Одна узкая клетка — один календарный день</span><span>Актуально на ' + escapeHtml(printedAt) + '</span></footer>' +
            '</div></section>');
        }
        ...
        return '<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>График производства — ' + escapeHtml(projectTitle) + '</title><style>' +
            ...
            '.production-print-legend{align-items:center;display:flex;flex-wrap:wrap;gap:2mm 4mm;margin:0 0 2mm}.production-print-legend span{align-items:center;color:#4e5963;display:inline-flex;font-size:6.2pt;gap:1mm}.production-print-legend i{border:.2mm solid rgba(24,33,42,.25);display:inline-block;height:2.4mm;width:2.4mm}.production-print-legend .is-neutral{background:#a5adb4}.production-print-legend .is-green{background:#4d9168}.production-print-legend .is-yellow{background:#d4a62d}.production-print-legend .is-red{background:#c94f4f}' +
            ...
            '.production-print-month{background:#dce3e8!important;font-size:6.2pt!important;text-transform:uppercase}.production-print-day small{color:#59636c;display:block;font-size:4.8pt;font-weight:600;margin-top:.4mm}.production-print-day.is-weekend{background:#f1f2f2!important}.production-print-day.is-today{box-shadow:inset 0 -.55mm #2f6fda}' +
            '.production-print-table tr{break-inside:avoid;page-break-inside:avoid}.production-print-table .production-print-title{font-size:6.5pt;text-align:left;word-break:break-word}.production-print-health{display:block;font-size:5pt;font-weight:700;margin-top:.5mm}.production-print-health.is-neutral{color:#66717b}.production-print-health.is-green{color:#34714d}.production-print-health.is-yellow{color:#896600}.production-print-health.is-red{color:#a23535}' +
            ...
            '.production-print-slot{padding:0!important}.production-print-slot.is-filled.tone-neutral{background:#a5adb4}.production-print-slot.is-filled.tone-green{background:#4d9168}.production-print-slot.is-filled.tone-yellow{background:#d4a62d}.production-print-slot.is-filled.tone-red{background:#c94f4f}.production-print-slot.is-partial{background:linear-gradient(90deg,var(--production-print-partial,#a5adb4) 0 50%,#fff 50% 100%)}.production-print-slot.is-partial.tone-green{--production-print-partial:#4d9168}.production-print-slot.is-partial.tone-yellow{--production-print-partial:#d4a62d}.production-print-slot.is-partial.tone-red{--production-print-partial:#c94f4f}' +
            '.production-print-empty td{color:#68737d;font-style:italic;padding:5mm}' +
    ...
    function renderProductionSchedule(project, schedule) {
        ...
        var startDateControl = canEditSchedule
            ...
            '</div>'
            : '';
        var hiddenColumnCount = Object.keys(hiddenColumns).length;
        var columnSettingsControl = canEditSchedule
            ? '<details class="production-column-settings" data-production-column-settings>' +
                '<summary><i data-lucide="columns-3" aria-hidden="true"></i><span>Колонки' + (hiddenColumnCount ? ' · скрыто ' + String(hiddenColumnCount) : '') + '</span></summary>' +
                '<div class="production-column-settings-menu"><strong>Показывать в графике</strong>' +
                    '<label><input type="checkbox" data-production-column-toggle="people"' + (hiddenColumns.people ? '' : ' checked') + '><span>Количество человек</span></label>' +
                    '<label><input type="checkbox" data-production-column-toggle="shifts"' + (hiddenColumns.shifts ? '' : ' checked') + '><span>Количество смен</span></label>' +
                    '<label><input type="checkbox" data-production-column-toggle="brigades"' + (hiddenColumns.brigades ? '' : ' checked') + '><span>Количество бригад</span></label>' +
                    '<small>Колонки скрываются для всех, кто смотрит этот график.</small>' +
                '</div>' +
            '</details>'
            : '';
        ...
        var optionalColumnHeaders = (hiddenColumns.people ? '' : '<th class="production-people-cell" rowspan="2" data-production-column-header="people" data-production-column-label="Количество человек" tabindex="0" title="Правая кнопка мыши — скрыть колонку">Кол-во<br>чел.</th>') +
            ...
            estimate.sections.forEach(function (section) {
                if (viewMode === 'sections') {
                    displayEntries.push({
                        ...
                        estimate: estimate,
                        section: section,
                        item: productionScheduleSectionSummary(estimate, section, summaryIndex += 1, schedule)
                    });
                } else {
        ...
        displayEntries.forEach(function (entry) {
            if (entry.type === 'estimate') {
                var estimateContext = canEditSchedule && entry.estimate.sourceId != null
                    ? ' data-production-context-kind="estimate" data-estimate-source-id="' + escapeHtml(entry.estimate.sourceId) + '" data-current-title="' + escapeHtml(entry.estimate.title) + '" data-show-estimate-label="' + (showEstimateLabel ? '1' : '0') + '" tabindex="0" title="Правая кнопка мыши — настроить смету"'
                    : '';
                rows.push('<tr class="production-estimate-row"' + estimateContext + '><th colspan="' + String(fixedColumnCount + visibleDays) + '"><span>' + (showEstimateLabel ? '<small>Смета</small>' : '') + '<b>' + escapeHtml(entry.estimate.title) + '</b></span></th></tr>');
                return;
            }
            if (entry.type === 'section') {
                var sectionHeaderSummary = productionScheduleSectionSummary(entry.estimate, entry.section, 0, schedule);
                var sectionContext = canEditSchedule && (entry.estimate.sourceId != null || sectionHeaderSummary.manualOperationId != null)
                    ? ' data-production-context-kind="section" data-estimate-source-id="' + escapeHtml(entry.estimate.sourceId == null ? '' : entry.estimate.sourceId) + '" data-manual-operation-id="' + escapeHtml(sectionHeaderSummary.manualOperationId == null ? '' : sectionHeaderSummary.manualOperationId) + '" data-current-title="' + escapeHtml(entry.section.rawTitle) + '" data-display-title="' + escapeHtml(entry.section.title) + '" data-operation-count="' + escapeHtml(sectionHeaderSummary.operationCount || 0) + '" data-duration-days="' + escapeHtml(sectionHeaderSummary.durationDays || 0.5) + '" data-start-date="' + escapeHtml(sectionHeaderSummary.plannedStartDate || '') + '" data-auto-start-date="' + escapeHtml(sectionHeaderSummary.autoStartDate || '') + '" data-start-overridden="' + (sectionHeaderSummary.isStartOverridden ? '1' : '0') + '" data-planned-qty="' + escapeHtml(sectionHeaderSummary.plannedQty == null ? '' : sectionHeaderSummary.plannedQty) + '" data-unit="' + escapeHtml(sectionHeaderSummary.unit || '') + '" data-calculated-qty="' + escapeHtml(sectionHeaderSummary.calculatedPlannedQty == null ? '' : sectionHeaderSummary.calculatedPlannedQty) + '" data-calculated-unit="' + escapeHtml(sectionHeaderSummary.calculatedUnit || '') + '" data-volume-overridden="' + (sectionHeaderSummary.isVolumeOverridden ? '1' : '0') + '" tabindex="0" title="Правая кнопка мыши — изменить или удалить раздел"'
                    : '';
                rows.push('<tr class="production-section-row"' + sectionContext + '><th colspan="' + String(fixedColumnCount + visibleDays) + '"><span>' + escapeHtml(entry.section.title) + '</span></th></tr>');
                return;
            }
            var item = entry.item;
            ...
            for (var cellDay = 1; cellDay <= visibleDays; cellDay += 1) {
                ...
                var secondSlotNumber = firstSlotNumber + 1;
                var filledSlotCount = (filled[String(firstSlotNumber)] ? 1 : 0) + (filled[String(secondSlotNumber)] ? 1 : 0);
                var isFilled = filledSlotCount > 0;
            ...
            var rawVolume = item.plannedQty != null ? item.plannedQty : item.planned_qty;
            ...
            var dragHandle = canEditSchedule && !isSectionSummary
                ...
                : '';
            var rowContext = '';
            if (canEditSchedule && isSectionSummary && (entry.estimate.sourceId != null || item.manualOperationId != null)) {
                rowContext = ' data-production-context-kind="section" data-estimate-source-id="' + escapeHtml(entry.estimate.sourceId == null ? '' : entry.estimate.sourceId) + '" data-manual-operation-id="' + escapeHtml(item.manualOperationId == null ? '' : item.manualOperationId) + '" data-current-title="' + escapeHtml(entry.section.rawTitle) + '" data-display-title="' + escapeHtml(entry.section.title) + '" data-operation-count="' + escapeHtml(item.operationCount || 0) + '" data-duration-days="' + escapeHtml(item.durationDays || 0.5) + '" data-start-date="' + escapeHtml(item.plannedStartDate || '') + '" data-auto-start-date="' + escapeHtml(item.autoStartDate || '') + '" data-start-overridden="' + (item.isStartOverridden ? '1' : '0') + '" data-planned-qty="' + escapeHtml(item.plannedQty == null ? '' : item.plannedQty) + '" data-unit="' + escapeHtml(item.unit || '') + '" data-calculated-qty="' + escapeHtml(item.calculatedPlannedQty == null ? '' : item.calculatedPlannedQty) + '" data-calculated-unit="' + escapeHtml(item.calculatedUnit || '') + '" data-volume-overridden="' + (item.isVolumeOverridden ? '1' : '0') + '" tabindex="0" title="Правая кнопка мыши — изменить или удалить раздел"';
    ...
    function saveProductionVolumeValue(projectId, input) {
        var rawValue = String(input.value == null ? '' : input.value).trim();
        var normalized = rawValue.replace(',', '.');
        var plannedQty = rawValue === '' ? null : Number(normalized);
        var previousValue = input.dataset.initialValue == null ? '' : input.dataset.initialValue;
        if (plannedQty !== null && (!Number.isFinite(plannedQty) || plannedQty < 0)) {
            input.value = previousValue;
            showAppNotice('Объём должен быть числом не меньше нуля.', 'error');
            return Promise.resolve(null);
        }
        var comparable = plannedQty == null ? '' : String(plannedQty);
        var previousComparable = String(previousValue || '').replace(',', '.');
        if (comparable === previousComparable) return Promise.resolve(null);
        input.value = comparable;
        input.disabled = true;
        return saveProductionScheduleAction(projectId, {
            action: 'update_operation',
            operation_id: productionPayloadId(input.dataset.operationId),
            planned_qty: plannedQty
        }, null).then(function (schedule) {
            showAppNotice('Объём работы сохранён.', 'success');
            return schedule;
        }).catch(function (error) {
            input.value = previousValue;
    ...
    function productionOperationFormValues(form) {
        ...
        return {
            ...
            startDate: productionScheduleIsoDate(form.elements.start_date.value),
            status: String(form.elements.status.value || 'needs_review').trim(),
            color: String(form.elements.color.value || 'slate').trim(),
    ...
    function productionOperationItemValues(item, selectedLinkIds) {
        ...
        return {
            ...
            startDate: productionScheduleIsoDate(item && item.plannedStartDate),
            status: String(item && item.status || 'needs_review').trim(),
            color: String(item && (item.colorKey || item.color) || 'slate').trim(),
    ...
    function openProductionGroupEditor(target) {
        ...
        form._productionManualOperationId = productionPayloadId(target.dataset.manualOperationId || '');
        form._productionInitialTitle = currentTitle;
        form._productionResetVolume = false;
        form._productionInitialVolumeOverridden = target.dataset.volumeOverridden === '1';
        form._productionInitialPlannedQty = String(target.dataset.plannedQty || '');
        form._productionInitialUnit = String(target.dataset.unit || '');
        form._productionCalculatedPlannedQty = String(target.dataset.calculatedQty || '');
        form._productionCalculatedUnit = String(target.dataset.calculatedUnit || '');
        form._productionOperationCount = Math.max(0, Number(target.dataset.operationCount || 0));
        form._productionInitialDurationDays = String(target.dataset.durationDays || '');
        form._productionInitialStartDate = productionScheduleIsoDate(target.dataset.startDate);
        form._productionAutoStartDate = productionScheduleIsoDate(target.dataset.autoStartDate) || form._productionInitialStartDate;
        form._productionInitialStartOverridden = target.dataset.startOverridden === '1';
        form._productionResetStartDate = false;
        form._productionInitialShowEstimateLabel = target.dataset.showEstimateLabel !== '0';
        if (form.elements.planned_qty) form.elements.planned_qty.value = kind === 'section' ? form._productionInitialPlannedQty : '';
        if (form.elements.unit) form.elements.unit.value = kind === 'section' ? form._productionInitialUnit : '';
        if (form.elements.section_duration_days) {
            form.elements.section_duration_days.value = kind === 'section' ? (form._productionInitialDurationDays || '0.5') : '0.5';
            form.elements.section_duration_days.min = String(Math.max(0.5, form._productionOperationCount / 2));
        }
        if (form.elements.section_start_date) {
            var currentSchedule = state.productionScheduleByProject && state.productionScheduleByProject[state.selectedProject && state.selectedProject.id];
            var scheduleStartDate = productionScheduleStartDate(state.selectedProject, currentSchedule);
            form.elements.section_start_date.min = scheduleStartDate;
            form.elements.section_start_date.value = kind === 'section'
                ? (form._productionInitialStartDate || scheduleStartDate)
                : scheduleStartDate;
        }
        if (form.elements.show_estimate_label) form.elements.show_estimate_label.checked = form._productionInitialShowEstimateLabel;
        ...
        var volumeFields = qs('[data-production-section-volume-fields]', form);
        var volumeHelp = qs('[data-production-section-volume-help]', form);
        var volumeReset = qs('[data-production-section-volume-reset]', form);
        var deleteSection = qs('[data-production-delete-section]', form);
        var startDateReset = qs('[data-production-section-start-reset]', form);
        var estimateLabelField = qs('[data-production-estimate-label-field]', form);
        if (heading) heading.textContent = kind === 'section' ? 'Настройка раздела' : 'Название сметы';
        if (description) description.textContent = kind === 'section' ? 'Название применяется ко всем работам раздела, а сводный объём — только к строке раздела в графике.' : 'Изменение применится к этой смете в объекте.';
        if (label) label.textContent = kind === 'section' ? 'Название раздела' : 'Новое название сметы';
        if (volumeFields) volumeFields.hidden = kind !== 'section';
        if (volumeHelp) volumeHelp.textContent = 'Сводное значение не меняет объёмы отдельных работ. Если единицы у работ разные, задайте здесь понятный общий объём вручную.';
        if (volumeReset) volumeReset.hidden = kind !== 'section' || !form._productionInitialVolumeOverridden;
        if (deleteSection) deleteSection.hidden = kind !== 'section';
        if (startDateReset) startDateReset.hidden = kind !== 'section' || !form._productionInitialStartOverridden;
        if (estimateLabelField) estimateLabelField.hidden = kind !== 'estimate';
    ...
    function openProductionOperationEditor(projectId, operationId, createKind) {
        var panel = qs('[data-panel="production-schedule"]');
        ...
        var isCreatingSection = !item && createKind === 'section';
        var currentSchedule = state.productionScheduleByProject && state.productionScheduleByProject[projectId];
        var scheduleStartDate = productionScheduleStartDate(state.selectedProject, currentSchedule);
        var automaticStartDate = item
            ? (productionScheduleIsoDate(item.autoStartDate) || productionScheduleIsoDate(item.plannedStartDate) || scheduleStartDate)
            : productionScheduleAddDays(scheduleStartDate, Math.ceil(Number(currentSchedule && currentSchedule.autoSlotCount || 0) / 2));
        var currentStartDate = item
            ? (productionScheduleIsoDate(item.plannedStartDate) || automaticStartDate)
            : automaticStartDate;
        ...
        setValue('start_date', currentStartDate);
        if (form.elements.start_date) form.elements.start_date.min = scheduleStartDate;
        setValue('status', item ? (item.status || 'needs_review') : 'needs_review');
        setValue('color', item ? (item.colorKey || item.color || 'slate') : 'slate');
        ...
        form._productionInitialStartDate = currentStartDate;
        form._productionAutoStartDate = automaticStartDate;
        form._productionResetStartDate = false;
        form._productionStartDateTouched = false;
        form._productionIsNew = !item;
        form._productionCreatingSection = isCreatingSection;
        ...
        if (heading) heading.textContent = item ? 'Редактирование работы' : (isCreatingSection ? 'Новый раздел' : 'Новая работа');
        var description = qs('[data-production-editor-description]', drawer);
        if (description) description.textContent = isCreatingSection
            ? 'Раздел появится в отдельной группе «Добавленные вручную» и сразу получит даты на графике.'
            : 'Операция может быть связана с одной или несколькими строками сметы — работами или материалами.';
        var titleLabel = qs('[data-production-title-label]', form);
        if (titleLabel) titleLabel.textContent = isCreatingSection ? 'Название раздела' : 'Наименование работы';
        ['[data-production-team-fields]', '[data-production-state-fields]', '[data-production-estimate-links]'].forEach(function (selector) {
            var field = qs(selector, form);
            if (field) field.hidden = isCreatingSection;
        });
        var submitLabel = qs('[data-production-editor-submit] span', form);
        if (submitLabel) submitLabel.textContent = isCreatingSection ? 'Добавить раздел' : 'Сохранить работу';
        var error = qs('[data-production-operation-error]', form);
        if (error) error.textContent = '';
        var startDateReset = qs('[data-production-operation-start-reset]', form);
        if (startDateReset) startDateReset.hidden = !item || String(item.placementMode || '').toLowerCase() !== 'manual';
        drawer.setAttribute('aria-hidden', 'false');
    ...
    function productionOperationFormPayload(form) {
        ...
        if (!operationId) {
            var createPayload = {
                action: form._productionCreatingSection ? 'add_section' : 'add_operation',
                title: values.title,
            ...
            if (form._productionStartDateTouched) createPayload.start_date = values.startDate;
        ...
        if (form._productionResetStartDate) payload.reset_start_date = true;
        else if (values.startDate !== initial.startDate) payload.start_date = values.startDate;
        if (values.status !== initial.status) payload.status = values.status;
        if (values.color !== initial.color) payload.color = values.color;
    ...
    function bindProductionScheduleInteractions(projectId) {
        ...
        var startDateInput = qs('[data-production-start-date]', panel);
        var saveStartDate = function (nextStartDate, control) {
            var normalized = productionScheduleIsoDate(nextStartDate);
            if (!normalized || normalized !== String(nextStartDate || '')) {
                showAppNotice('Укажите корректную дату начала графика.', 'error');
                if (startDateInput) startDateInput.value = productionScheduleStartDate(state.selectedProject, state.productionScheduleByProject[projectId]);
                return Promise.resolve(null);
            }
            return saveProductionScheduleAction(projectId, {
                action: 'set_start_date',
                start_date: normalized
            }, control).then(function (schedule) {
                if (schedule) showAppNotice('Начало графика: ' + productionScheduleDayMeta(normalized, 1, schedule.today).shortLabel + '.', 'success');
                return schedule;
            });
        };
        if (startDateInput) {
            startDateInput.addEventListener('change', function () {
                saveStartDate(startDateInput.value, startDateInput).catch(function () {});
            });
        }
        qsa('[data-production-start-shift]', panel).forEach(function (button) {
            button.addEventListener('click', function () {
                var currentStart = startDateInput ? startDateInput.value : productionScheduleStartDate(state.selectedProject, state.productionScheduleByProject[projectId]);
                var nextStart = productionScheduleAddDays(currentStart, Number(button.dataset.productionStartShift || 0));
                if (startDateInput) startDateInput.value = nextStart;
                saveStartDate(nextStart, button).catch(function () {});
            });
        });
        qsa('[data-production-start-reset]', panel).forEach(function (button) {
            button.addEventListener('click', function () {
                saveProductionScheduleAction(projectId, { action: 'set_start_date', reset: true }, button).then(function () {
                    showAppNotice('Начало графика снова берётся из даты объекта.', 'success');
                }).catch(function () {});
            });
        });
        var saveVisibleColumns = function (hiddenColumnKeys, control) {
            return saveProductionScheduleAction(projectId, {
                action: 'update_display_settings',
                hidden_columns: hiddenColumnKeys
            }, control).then(function (schedule) {
                if (schedule) showAppNotice('Состав колонок графика обновлён.', 'success');
                return schedule;
            });
        };
        qsa('[data-production-column-toggle]', panel).forEach(function (input) {
            input.addEventListener('change', function () {
                var hiddenColumnKeys = qsa('[data-production-column-toggle]', panel).filter(function (columnInput) {
                    return !columnInput.checked;
                }).map(function (columnInput) {
                    return columnInput.dataset.productionColumnToggle;
                });
                saveVisibleColumns(hiddenColumnKeys, input).catch(function () {});
            });
        });
        var columnContextMenu = qs('[data-production-column-context-menu]', panel);
        var closeColumnContextMenu = function () {
            if (columnContextMenu) columnContextMenu.hidden = true;
        };
        var openColumnContextMenu = function (header, event) {
            if (!columnContextMenu || !header) return;
            if (event) event.preventDefault();
            var rect = header.getBoundingClientRect();
            var left = event && Number.isFinite(event.clientX) ? event.clientX : rect.left;
            var top = event && Number.isFinite(event.clientY) ? event.clientY : rect.bottom;
            columnContextMenu.dataset.productionColumn = header.dataset.productionColumnHeader || '';
            var title = qs('[data-production-column-context-title]', columnContextMenu);
            if (title) title.textContent = header.dataset.productionColumnLabel || 'Колонка';
            columnContextMenu.style.left = Math.max(8, Math.min(left, window.innerWidth - 220)) + 'px';
            columnContextMenu.style.top = Math.max(8, Math.min(top, window.innerHeight - 96)) + 'px';
            columnContextMenu.hidden = false;
            var hideButton = qs('[data-production-hide-column]', columnContextMenu);
            if (hideButton && typeof hideButton.focus === 'function') hideButton.focus();
            window.setTimeout(function () {
                document.addEventListener('click', closeColumnContextMenu, { once: true });
            }, 0);
        };
        qsa('[data-production-column-header]', panel).forEach(function (header) {
            header.addEventListener('contextmenu', function (event) { openColumnContextMenu(header, event); });
            header.addEventListener('keydown', function (event) {
                if (event.key !== 'ContextMenu' && !(event.shiftKey && event.key === 'F10')) return;
                openColumnContextMenu(header, event);
            });
        });
        var hideColumnButton = columnContextMenu ? qs('[data-production-hide-column]', columnContextMenu) : null;
        if (hideColumnButton) {
            hideColumnButton.addEventListener('click', function (event) {
                event.stopPropagation();
                var columnKey = String(columnContextMenu.dataset.productionColumn || '');
                var hidden = productionScheduleHiddenColumnSet(state.productionScheduleByProject[projectId]);
                hidden[columnKey] = true;
                closeColumnContextMenu();
                saveVisibleColumns(Object.keys(hidden), hideColumnButton).catch(function () {});
            });
        }
        qsa('[data-production-edit-schedule]', panel).forEach(function (button) {
            if (button.dataset.bound === '1') return;
            button.dataset.bound = '1';
            button.addEventListener('click', function () {
                state.productionScheduleViewByProject = state.productionScheduleViewByProject || {};
        ...
        qsa('[data-production-day]', panel).forEach(function (button) {
            if (button.dataset.bound === '1') return;
            button.dataset.bound = '1';
            button.addEventListener('click', function () {
        ...
        qsa('[data-production-volume]', panel).forEach(function (input) {
            if (input.dataset.bound === '1') return;
            input.dataset.bound = '1';
            input.dataset.initialValue = input.value;
            input.addEventListener('change', function () {
                saveProductionVolumeValue(projectId, input).catch(function () {});
            });
            input.addEventListener('keydown', function (event) {
        ...
        qsa('[data-production-add-section]', panel).forEach(function (button) {
            button.addEventListener('click', function () { openProductionOperationEditor(projectId, null, 'section'); });
        });
        ...
        if (operationForm) {
            var operationStartDateInput = operationForm.elements.start_date;
            var operationStartReset = qs('[data-production-operation-start-reset]', operationForm);
            if (operationStartDateInput) {
                operationStartDateInput.addEventListener('change', function () {
                    operationForm._productionResetStartDate = false;
                    operationForm._productionStartDateTouched = true;
                    if (operationStartReset) operationStartReset.hidden = false;
                });
            }
            if (operationStartReset) {
                operationStartReset.addEventListener('click', function () {
                    if (operationStartDateInput) operationStartDateInput.value = operationForm._productionAutoStartDate || operationStartDateInput.min;
                    operationForm._productionResetStartDate = !operationForm._productionIsNew;
                    operationForm._productionStartDateTouched = !operationForm._productionIsNew;
                    operationStartReset.hidden = true;
                });
            }
            operationForm.addEventListener('submit', function (event) {
                ...
                if (!String(operationForm.elements.title.value || '').trim()) {
                    if (error) error.textContent = operationForm._productionCreatingSection ? 'Укажите название раздела.' : 'Укажите наименование работы.';
                    return;
                }
                var rawDuration = String(operationForm.elements.duration_days.value || '').trim().replace(',', '.');
                var parsedDuration = Number(rawDuration);
                if (!rawDuration || !Number.isFinite(parsedDuration) || parsedDuration < 0.5 || parsedDuration > 3650 || Math.round(parsedDuration * 2) !== parsedDuration * 2) {
                    if (error) error.textContent = 'Продолжительность должна быть от 0,5 дня и меняться шагом 0,5.';
                    return;
                }
                var normalizedStartDate = productionScheduleIsoDate(operationForm.elements.start_date.value);
                var minimumStartDate = productionScheduleIsoDate(operationForm.elements.start_date.min);
                if (!normalizedStartDate || (minimumStartDate && normalizedStartDate < minimumStartDate)) {
                    if (error) error.textContent = 'Дата начала работы не может быть раньше начала всего графика.';
                    return;
                }
                ...
                saveProductionScheduleAction(projectId, payload, submit).then(function () {
                    closeProductionOperationEditor();
                    showAppNotice(payload.action === 'add_section' ? 'Раздел добавлен в график.' : (payload.action === 'add_operation' ? 'Работа добавлена в график.' : 'Работа обновлена.'), 'success');
        ...
        if (groupEditorForm) {
            var deleteSectionButton = qs('[data-production-delete-section]', groupEditorForm);
            if (deleteSectionButton) {
                deleteSectionButton.addEventListener('click', function () {
                    var sectionTitle = String(groupEditorForm.elements.title.value || groupEditorForm._productionInitialTitle || 'Без раздела').trim();
                    var operationCount = Math.max(0, Number(groupEditorForm._productionOperationCount || 0));
                    var countLabel = operationCount ? ' Вместе с ним из графика исчезнет работ: ' + String(operationCount) + '.' : '';
                    if (!window.confirm('Удалить раздел «' + sectionTitle + '» из графика?' + countLabel + ' Исходная смета и её позиции останутся без изменений.')) return;
                    var manualOperationId = productionPayloadId(groupEditorForm._productionManualOperationId || '');
                    var deletePayload = manualOperationId
                        ? { action: 'delete_operation', operation_id: manualOperationId }
                        : {
                            action: 'delete_section',
                            estimate_source_id: productionPayloadId(groupEditorForm.elements.estimate_source_id.value),
                            section_title: groupEditorForm.elements.old_title.value
                        };
                    saveProductionScheduleAction(projectId, deletePayload, deleteSectionButton).then(function () {
                        closeProductionGroupEditor();
                        showAppNotice(manualOperationId ? 'Добавленный раздел удалён из графика.' : 'Раздел удалён из графика. Смета не изменена.', 'success');
                    }).catch(function (saveError) {
                        var error = qs('[data-production-group-editor-error]', groupEditorForm);
                        if (error) error.textContent = appErrorMessage(saveError, 'Не удалось удалить раздел.');
                    });
                });
            }
            var groupVolumeReset = qs('[data-production-section-volume-reset]', groupEditorForm);
            var groupStartDateInput = groupEditorForm.elements.section_start_date;
            var groupStartDateReset = qs('[data-production-section-start-reset]', groupEditorForm);
            if (groupStartDateInput) {
                groupStartDateInput.addEventListener('change', function () {
                    groupEditorForm._productionResetStartDate = false;
                    if (groupStartDateReset) groupStartDateReset.hidden = false;
                });
            }
            if (groupStartDateReset) {
                groupStartDateReset.addEventListener('click', function () {
                    if (groupStartDateInput) groupStartDateInput.value = groupEditorForm._productionAutoStartDate || groupStartDateInput.min;
                    groupEditorForm._productionResetStartDate = true;
                    groupStartDateReset.hidden = true;
                });
            }
            var clearPendingVolumeReset = function () {
                if (!groupEditorForm._productionResetVolume) return;
                groupEditorForm._productionResetVolume = false;
                var help = qs('[data-production-section-volume-help]', groupEditorForm);
                if (help) help.textContent = 'Сводное значение не меняет объёмы отдельных работ. Если единицы у работ разные, задайте здесь понятный общий объём вручную.';
                if (groupVolumeReset) groupVolumeReset.hidden = !groupEditorForm._productionInitialVolumeOverridden;
            };
            ['planned_qty', 'unit'].forEach(function (name) {
                if (groupEditorForm.elements[name]) groupEditorForm.elements[name].addEventListener('input', clearPendingVolumeReset);
            });
            if (groupVolumeReset) {
                groupVolumeReset.addEventListener('click', function () {
                    groupEditorForm._productionResetVolume = true;
                    groupEditorForm.elements.planned_qty.value = groupEditorForm._productionCalculatedPlannedQty || '';
                    groupEditorForm.elements.unit.value = groupEditorForm._productionCalculatedUnit || '';
                    groupVolumeReset.hidden = true;
                    var help = qs('[data-production-section-volume-help]', groupEditorForm);
                    if (help) help.textContent = 'После сохранения объём снова будет рассчитываться по вложенным работам.';
                });
            }
            groupEditorForm.addEventListener('submit', function (event) {
                ...
                var payload = {
                    action: kind === 'section' && groupEditorForm._productionManualOperationId ? 'update_operation' : (kind === 'section' ? 'update_section' : 'rename_estimate')
                };
                if (kind === 'section') {
                    var manualSectionOperationId = productionPayloadId(groupEditorForm._productionManualOperationId || '');
                    if (manualSectionOperationId) {
                        payload.operation_id = manualSectionOperationId;
                        if (nextTitle !== groupEditorForm._productionInitialTitle) {
                            payload.title = nextTitle;
                            payload.section_title = nextTitle;
                        }
                    } else {
                        payload.estimate_source_id = productionPayloadId(groupEditorForm.elements.estimate_source_id.value);
                        payload.old_title = groupEditorForm.elements.old_title.value;
                        if (nextTitle !== groupEditorForm._productionInitialTitle) payload.title = nextTitle;
                    }
                    var nextQtyRaw = String(groupEditorForm.elements.planned_qty.value || '').trim().replace(',', '.');
                    var nextUnit = String(groupEditorForm.elements.unit.value || '').trim();
                    if (groupEditorForm._productionResetVolume) {
                        payload.reset_volume = true;
                    } else if (nextQtyRaw !== groupEditorForm._productionInitialPlannedQty || nextUnit !== groupEditorForm._productionInitialUnit) {
                        var nextQty = Number(nextQtyRaw);
                        if (!nextQtyRaw || !Number.isFinite(nextQty) || nextQty < 0) {
                            if (error) error.textContent = 'Укажите сводный объём числом не меньше нуля.';
                            return;
                        }
                        payload.planned_qty = nextQty;
                        payload.unit = nextUnit;
                    }
                    var nextDurationRaw = String(groupEditorForm.elements.section_duration_days.value || '').trim().replace(',', '.');
                    var nextDuration = Number(nextDurationRaw);
                    var initialDuration = Number(groupEditorForm._productionInitialDurationDays || 0);
                    var minimumDuration = Math.max(0.5, Number(groupEditorForm.elements.section_duration_days.min || 0.5));
                    if (!Number.isFinite(nextDuration) || nextDuration < minimumDuration || Math.round(nextDuration * 2) !== nextDuration * 2) {
                        if (error) error.textContent = 'Продолжительность раздела должна быть не меньше ' + quantityText(minimumDuration) + ' дн. и меняться шагом 0,5 дня.';
                        return;
                    }
                    if (nextDuration !== initialDuration) payload.duration_days = nextDuration;
                    var nextStartDate = productionScheduleIsoDate(groupEditorForm.elements.section_start_date.value);
                    var minimumStartDate = productionScheduleIsoDate(groupEditorForm.elements.section_start_date.min);
                    if (!nextStartDate || (minimumStartDate && nextStartDate < minimumStartDate)) {
                        if (error) error.textContent = 'Дата начала раздела не может быть раньше начала всего графика.';
                        return;
                    }
                    if (groupEditorForm._productionResetStartDate) payload.reset_start_date = true;
                    else if (nextStartDate !== groupEditorForm._productionInitialStartDate) payload.start_date = nextStartDate;
                    if (Object.keys(payload).length === (manualSectionOperationId ? 2 : 3)) {
                        closeProductionGroupEditor();
                        return;
                    }
                } else {
                    payload.estimate_source_id = productionPayloadId(groupEditorForm.elements.estimate_source_id.value);
                    var nextShowEstimateLabel = !!groupEditorForm.elements.show_estimate_label.checked;
                    if (nextTitle === groupEditorForm._productionInitialTitle && nextShowEstimateLabel === groupEditorForm._productionInitialShowEstimateLabel) {
                        closeProductionGroupEditor();
                        return;
                    }
                    payload.title = nextTitle;
                    if (nextShowEstimateLabel !== groupEditorForm._productionInitialShowEstimateLabel) payload.show_estimate_label = nextShowEstimateLabel;
                }
                var submit = qs('button[type="submit"]', groupEditorForm);
                if (error) error.textContent = '';
                saveProductionScheduleAction(projectId, payload, submit).then(function () {
                    closeProductionGroupEditor();
                    showAppNotice(kind === 'section' ? 'Раздел, даты и длительность обновлены.' : 'Название сметы обновлено.', 'success');
                }).catch(function (saveError) {
                    if (error) error.textContent = appErrorMessage(saveError, kind === 'section' ? 'Не удалось сохранить раздел.' : 'Не удалось сохранить название.');
                });
            });
        }
