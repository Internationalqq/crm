...
Suffix `-mobile` identifies a separately recorded portrait walkthrough, not a crop of the desktop recording. The page footer labels the recordings as demonstration data. Tabs advance when the selected recording ends and can be selected manually or from the keyboard. No pause controls, as requested. Product recordings stay as readable stills for reduced motion, Save Data, unavailable video or denied autoplay. Offscreen and hidden-page players are suspended. The separate enlarged view and repeated player footers were removed at the user's request on September 18. The construction hero retains the separately documented continuous-film policy above.
...
The Attio-direction review added four authentic recordings: `crm-report-focus[−mobile].mp4` and `bot-estimate-focus[−mobile].mp4` (the filenames use an ordinary hyphen). These replay the isolated applications at 900×632 desktop and 390×700 phone. CRM starts inside the report and opens its photograph. AutoBot parses the actual local Excel upload and scrolls to all four resulting positions; on the narrow screen it brings the name column into view. Nothing is mocked or written to production. The original 20 complete recordings and their posters remain unchanged as source assets.
