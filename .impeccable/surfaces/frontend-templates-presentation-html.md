...
## 19 сентября — будущие сценарии без раскрытий

По последнему polish-запросу все3concept-result удалены. Telegram — HTML-пример переписки прораба и AutoBot с фото и черновиком; условные данные и будущий статус явно указаны. Чек показывает результат сразу. Клубы/сети заменены на обслуживание строительного объекта, пример ремонта входной двери. Подпись стоимости центрируется. Сохраняются3вкладки future, записи и роли. Артефакты `tmp/presentation-future-20260919`.

## 19 сентября — последние четыре скриншота

Нижние раскрытые панели форматов удалены целиком. Три формата остаются статичными информационными колонками с иконками; старые tabs и их JS/CSS удалены. Надпись/toolbar PM.bi удалена, AutoBot сохранён. Workflow/plans выравнивают заголовок и пояснение по вертикальному центру. Роли — равные крупные кнопки76px, на телефоне64px в отдельных строках. Записи, автопоказ, будущие вкладки и FAQ сохраняются. Этот запрос заменяет прежнее требование сохранять интерактивные планы. Артефакты `tmp/presentation-layout-20260919`.

## 18 сентября — компактный текст и плавная навигация

Последняя правка: боковая навигация 216px, подписи 16px/400, иконки 18px, кнопки минимум56px. Уменьшенная типографика и более широкая колонка дают длинным названиям место. Полоска следует currentTime через единственный requestAnimationFrame; при buffering, pause, скрытии и reduced motion цикл остановлен. Остальные сцены и записи сохранены. Артефакты `tmp/presentation-smooth-20260918`.

## Последнее уточнение — убрать служебные элементы, добавить смысловые иконки

Четыре пользовательских кадра 18 сентября отменяют сохранение «Увеличить», «О сценарии» и нижних строк с «Демонстрационные данные»/ссылками. Эти элементы удаляются в обоих walkthrough вместе с неиспользуемым диалогом. Также убираются три figcaption ролей и три возвратные ссылки «Посмотреть действия в CRM». Общий footer продолжает обозначать условность записей; будущие функции остаются явно будущими. Добавляются единые SVG-иконки: Telegram, чек, команда, каска, дом, существующие документы/цены/связь. Записи, обычная скорость, вкладки, автоматическая смена и светлая среда сохраняются. В future-сцене колонки выровнены по верху, фото занимает ширину своего блока вместо ограничения 360px; изображение, header и disclosure имеют общие края. Артефакты: `tmp/presentation-trim-20260918`. Эта инструкция имеет приоритет над прежним требованием сохранять увеличение и раскрываемое описание.
...
## Текущая правка — видимые элементы управления страницы

Уточнение пользователя 18 сентября: «убери, видно куда курсор водится, я к видео вопросов не имею, я как раз про другие элементы говорил». Оно отменяет прошлый слой подсказок и его ship-verdict. Удалены guide script, рамки, синхронные подписи и кнопки шагов; возвращены исходный компактный плеер и скорость 1. Записи не редактируются. Уточнение касается самой страницы: главы, роли и будущие сценарии получают единое синее выбранное состояние; невыбранные роли/сценарии — видимые границы кнопок. Выбранный формат — синий с белым текстом. Раскрытия и FAQ имеют заметные hover/open состояния, крупный плюс и выделенное место нажатия. Ссылки подчёркнуты; увеличение и «О сценарии» оформлены как кнопки. Поведение, контент, реальная CRM и данные сохраняются. Пакет: `tmp/presentation-controls-20260918`. Проверяется 390/768/1280, короткое окно, все старые сценарии, отсутствие guide/замедления, клавиатура и читаемость. Никакой помощи поверх видео.
