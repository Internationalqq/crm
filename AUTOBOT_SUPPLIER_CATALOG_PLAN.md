...
- [x] Проверены текущие границы кода и сохранённый результат «Волги»; зафиксированы baseline и ограничения.
- [x] Подготовлен порядок К0–К12, первая демонстрация, модель данных и критерии приёмки.
- [ ] К0–К12: реализация не начата.

**Следующее задание — К0:** на копии текущей «Волги» собрать карту всех 127 строк с исходными местами, характеристиками и причинами пробелов; выделить контрольные потребности и приоритеты поставщиков. Результат — небольшой проверяемый отчёт и данные для К1, без изменения рабочей сметы. После него — проверка первоначальных 13 источников и выбор первого раздела каталога.

Код приложения и production в рамках подготовки этого плана не изменялись; тесты приложения и новые обходы сайтов не запускались. Проверка документа: сверка чисел с сохранённым результатом, модулей с текущим кодом, зависимостей пакетов и локальных ссылок. План уточняет развитие пакетов C–F из [спецификации модуля цен](AUTOBOT_PRICE_MODULE_SPEC.md); прежние требования к достоверности и [экономике проекта](PROJECT_ECONOMICS_SPEC.md) сохраняются.
