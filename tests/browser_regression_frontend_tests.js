...
const dashboardLoadStart = appJs.indexOf('function loadDashboard(callback)');
const dashboardLoadEnd = appJs.indexOf('var PROJECT_COMPANY_LABELS', dashboardLoadStart);
assert.ok(dashboardLoadStart >= 0 && dashboardLoadEnd > dashboardLoadStart, 'dashboard loader must exist');
const dashboardLoadBlock = appJs.slice(dashboardLoadStart, dashboardLoadEnd);
assert.match(dashboardLoadBlock, /state\.dashboardError = error/);
assert.match(dashboardLoadBlock, /callback\(error, state\.dashboard\)/);
assert.doesNotMatch(dashboardLoadBlock, /state\.dashboard = null/, 'a refresh failure must retain the last dashboard payload');
const dashboardInitStart = appJs.indexOf('function initDashboardPage()');
const dashboardInitEnd = appJs.indexOf('function renderDashboardPage(data)', dashboardInitStart);
assert.ok(dashboardInitStart >= 0 && dashboardInitEnd > dashboardInitStart, 'dashboard initialization block must exist');
const dashboardInitBlock = appJs.slice(dashboardInitStart, dashboardInitEnd);
assert.match(dashboardInitBlock, /renderDashboardLoadError\(error, !!data\)/);
assert.match(dashboardInitBlock, /settleDashboardLoadingFailure\(\)/);
assert.match(dashboardInitBlock, /setAttribute\('role', 'alert'\)/);
assert.match(dashboardInitBlock, /data-dashboard-retry/);
assert.match(dashboardInitBlock, /Показываем последние полученные данные/);
...
assert.match(dailyTasksHtml, /Ответственные, сроки и результаты работы команды/);
