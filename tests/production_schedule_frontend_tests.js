...
assert.match(planningJs, /function productionScheduleGroups/);
assert.match(planningJs, /function productionScheduleSectionSummary/);
assert.match(planningJs, /data-production-view="sections"/);
...
assert.match(planningJs, /Шаг 0,5 дня/);
...
// Touch users need an explicit edit action; the context menu remains available.
assert.match(planningJs, /var rowActions = canEditSchedule && !isSectionSummary[\s\S]*?data-production-edit-operation/);
assert.match(planningJs, /qsa\('\[data-production-edit-operation\]'[\s\S]*?openProductionOperationEditor\(projectId, button.dataset.operationId\)/);
assert.doesNotMatch(planningJs, /production-group-edit(?!or)|data-lucide="pencil"/);
...
assert.match(planningJs, /data-production-operation-form[^>]*novalidate/);
assert.match(planningJs, /name="start_date" type="date"/);
assert.match(planningJs, /name="section_start_date" type="date"/);
assert.match(planningJs, /Укажите дату, чтобы запустить эту работу параллельно/);
assert.match(planningJs, /Раздел можно начать раньше и вести параллельно/);
assert.match(planningJs, /createPayload\.start_date = values\.startDate/);
assert.match(planningJs, /payload\.reset_start_date = true/);
assert.match(planningJs, /payload\.start_date = nextStartDate/);
assert.match(planningJs, /Работа добавлена в график/);
...
assert.match(planningCss, /@media \(min-width: 721px\)[\s\S]*?\[data-panel="production-schedule"\]\.active \.production-schedule-card\s*\{[^}]*position: sticky;[^}]*top: \d+px;/s);
assert.match(planningCss, /\[data-panel="production-schedule"\]\.active \.production-table-shell\s*\{[^}]*overflow: hidden;/s);
assert.match(planningCss, /\[data-panel="production-schedule"\]\.active \.production-table-scroll\s*\{[^}]*max-height: calc\(100dvh - \d+px\);/s);
...
assert.match(planningCss, /\.production-schedule-table\s*\{[^}]*--production-number-width: \d+px;[^}]*--production-work-width: \d+px;/s);
...
assert.match(planningCss, /\.production-day-head\s*\{[^}]*width: var\(--production-day-width\);/s);
assert.match(planningCss, /\.production-day-cell\s*\{[^}]*width: var\(--production-day-width\);/s);
assert.match(planningCss, /\.production-cell-toggle\.is-filled\.is-partial::before\s*\{[^}]*linear-gradient/s);
...
assertVersionedAsset(rootCss, 'css/planning.css');
