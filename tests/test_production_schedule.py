...
class ProductionScheduleTests(unittest.TestCase):
    ...
    def test_production_schedule_is_sequential_then_applies_independent_cells(self):
        ...
        self.assertEqual(baseline["items"][0]["autoFilledDays"], [1, 2])
        self.assertEqual(baseline["items"][1]["autoFilledDays"], [3])

        con.executemany(
            "INSERT INTO production_schedule_cell_overrides VALUES (?, ?, ?, ?)",
            [(1, 11, 3, 0), (1, 11, 1, 1)],
        )
        edited = build_production_schedule_payload(con, 1)
        self.assertEqual(edited["items"][1]["filledDays"], [1])
        self.assertEqual(edited["items"][1]["effectiveDays"], 1)
    ...
    def test_production_duration_override_is_separate_from_graph_override(self):
        ...
        self.assertEqual(schedule["items"][1]["autoFilledDays"], [5])
